...
const emits = defineEmits(["clickResource"])
...
function handleChecked() {
    ...
    if (checked.value) {
        resourceRef.value.classList.add("resource--selected")
    } else {
        resourceRef.value.classList.remove("resource--selected")
    }
}
</script>

<template>
<div class="resource" ref="resourceRef">
    <div class="top">
        <div class="wrapper">
            <n-checkbox :checked="checked" :value="id" @click="handleChecked"/>
        </div>
    </div>
    <n-flex class="display" justify="center" @click="isFolder? emits('clickResource', id): null">
        <img v-if="isFolder" class="icon" alt=""
    ...
    <div class="name" @click="isFolder? emits('clickResource', id): null">
        <n-ellipsis :line-clamp="3" :tooltip="false">{{name}}</n-ellipsis>
    </div>
</div>
</template>
